       ...
       if(item.units && unit != positems[item_id].row.base_unit) {
            $.each(item.units, function(){
                if (this.id == unit) {
                    ...
                    var selected_unit = this.name;
                    if (item.row.puid) {
                        console.log(item.sale_unit);
                        $.each(item.sale_unit,function(){
                            if (selected_unit==this.sale_unitname) {
                                $('#pprice').val(formatDecimal(((parseFloat(this.sale_unitprice+aprice))*unitToBaseQty(1, this)), 4)).change();
                            }
                        });
...
function loadItems() {
    // console.log(site);
    var rate_cur = site.rate_cur;
    var munit = site.munit;
    ...
    if (localStorage.getItem('positems')) {
        ...
        if (pos_settings.remote_printing != 1) {
            ...
            // var pos_head1 = '<span style="text-align:center;"><h3>''</h3><h4>';
            var pos_head1 = '';
            var pos_head2 = '</h4><h6><div class="col-md-6 pull-left" style="text-align:left;font-size:11px !important;">លេខតុ: '+$('#select2-chosen-1').text()+'</div><div style="font-size:11px !important;" class="col-md-6 pull-right"><p class="text-left">Date: '+date(site.dateFormats.php_ldate, time)+'</p></div></h6></span>';
        ...
        $.each(sortedItems, function () {
            ...
            tr_html += '<input class="rprice" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + item_price + '"><input class="ruprice" name="unit_price[]" type="hidden" value="' + unit_price + '"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + item.row.real_unit_price + '"><span class="text-right sprice" id="sprice_' + row_no + '">' +parseFloat(item_price)+ '</span></td>';
            ...
            // tr_html += '<td><input class="form-control input-sm kb-pad text-center" name="quantity[]" type="text" value="' + formatQuantity2(item_qty) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();"></td>';
            
            tr_html += '<td class="text-right"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + ((parseFloat(item_price) + parseFloat(pr_tax_val)) * parseFloat(item_qty)) + '</span></td>';
...
// ======================================================= add  new for looping row ================================================;
                
                if (rows == 12) {
                    bprTr+='<tr style="page-break-after: always;"></tr>';
                }
                 
// ======================================================= add  new for looping row ================================================;

                var bprTr = '<tr style="font-size:11px !important; border:1px solid #000000 !important;" class="row_' + rows + '" data-item-id="' + rows + '"><td colspan="" style="text-align:center !important;border-right:1px solid #ddd;">'+ (an-1) + '</td>';               bprTr += '<td style="border-right:1px solid #ddd;">'+ item_name +'</td>';
                bprTr += '<td style="text-align:center !important;border-right:1px solid #ddd;">'+ formatDecimal(item_qty)+munit[product_unit] +'</td>';
                bprTr += '<td style="border-right:1px solid #ddd;"> R '+Number(parseFloat(item_price)+ parseFloat(pr_tax_val) + item_discount).toFixed(0) +'</td>';

                // if(item_discount == 0) {
                //     bprTr += '<td></td>';
                // } else if((formatMoney(((parseFloat(item_price) + parseFloat(pr_tax_val)) * parseFloat(item_qty)))) == 0) {
                //     bprTr += '<td style="text-align:center !important;">Free</td>';
                // } else {
                //     // bprTr += '<td style="text-align:center !important;"><del>'+ formatDecimal(item_qty)*item_discount +'</del></td>';
                // }
                // var discount=(item_discount != 0 ?  : '' ||  == '100%' ? 'Free':'');
                // var dis_item=item_discount;
                //  if (dis_item != 0) {
                //      // '<del>'+item_discount+'</del>'
                //      dis_item='<del>'+item_discount+'</del>';
                //  }else  if(dis_item ==  0){
                //      dis_item='';
                //  }else if (discount=='100%'){
                //      dis_item='Free';
                //  }
                // bprTr += '<td>'+dis_item+'</td>';

                bprTr += '<td style="text-align:right;">R '+Number( ((parseFloat(item_price) + parseFloat(pr_tax_val)) * parseFloat(item_qty))).toFixed(0)+'</td></tr>';
                rows++;
...
// ======================================add new for looping row table=========================================================
    // var bprTrr;
    //     for(cell=rows; cell<=17; cell++){

    //                 bprTrr+="<tr>";
    //                     bprTrr +="<td style='text-align:center !important;font-size:11px;'>"+cell+"</td>";
...
// ======================================add new for looping row table=========================================================
        ...
        if (pos_settings.remote_printing != 1) {
            ...
            var bill_totals = '';
            bill_totals += '<tr class=" fitem"><td style="text-align:center;font-size:8px;border-right:1px solid #ddd;border-left:1px solid #ddd;" rowspan=2>ថ្ងៃខែឆ្នាំចេញវិក័យប័ត្រ​ </br><div class="col-xs-6"><p style="font-size:8px;">អ្នកទិញ / The Buyer<hr></p></div> <div class="col-xs-6"><p style="font-size:8px;">អ្នកលក់ / The Seller<hr></p></div> </td><td style="font-size:8px;">សរុបរៀល</td><td style="font-size:8px !important;">R '+Number(total).toFixed(0)+'</td></tr>';
            
            if(order_discount > 0 || product_discount > 0) {
                bill_totals += '<tr class=" fitem"><td>'+lang.discount+'</td><td style="font-size:8px !important;text-align:right;">R '+Number(order_discount+product_discount).toFixed(0)+'</td></tr>';
            }
            if (site.settings.tax2 != 0 && invoice_tax != 0) {
                bill_totals += '<tr class=" fitem"><td>'+lang.order_tax+'</td><td style="font-size:8px !important;text-align:right;">R '+Number(invoice_tax).toFixed(0)+'</td></tr>';
            }
            // bill_totals += '<tr class=" fitem"><td>'+lang.grand_total+'</td><td style="font-size:11px !important;text-align:right;">R '+Number(gtotal).toFixed(0)+'</td></tr>';
            bill_totals += '<tr class=" fitem"><td style="font-size:8px;">សរុបដុល្លា</td><td style="font-size:8px !important;">$ '+Number(gtotal/rate_cur).toFixed(2)+'</td></tr>';
            if(pos_settings.rounding != 0) {
                round_total = roundNumber(gtotal, parseInt(pos_settings.rounding));
                var rounding = formatDecimal(round_total - gtotal);
                bill_totals += '<tr class=" fitem"><td>'+lang.rounding+'</td><td style="font-size:8px !important; text-align:right;border-top:1px solid #000 !important;">'+Number(rounding).toFixed(0)+'</td></tr>';
                bill_totals += '<tr class=" fitem"><td>'+lang.total_payable+'</td><td style="font-size:8px !important; text-align:right;border-top:1px solid #000 !important;">'+Number(round_total).toFixed(0)+'</td></tr>';
            }
            // bill_totals += '<tr class=" fitem"><td>'+lang.items+'</td><td style="font-size:11px !important; text-align:right;border-top:1px solid #000 !important;">'+(an - 1) + ' (' + (parseFloat(count) - 1) + ')</td></tr>';
 ...
 * @returns {Boolean}
 ...
 function add_invoice_item(item) {
    ...
    if (positems[item_id]) {
        ...
        // var unit = positems[item_id].row.product_unit_code;
        // alert(unit);
        positems[item_id].row.base_quantity = new_qty;
        if(positems[item_id].row.unit != positems[item_id].row.base_unit) {
            $.each(positems[item_id].units, function(){
