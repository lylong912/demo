        ...
        <div class="container">
            ...
            <div class="header-nav">
                ...
                <ul class="nav navbar-nav pull-right">
                    ...
                    <li class="dropdown">
                        <a class="btn pos-tip" title="<?= lang('view_phone_screen') ?>" data-placement="bottom" href="<?= admin_url('pos/phone_con') ?>" target="_blank">
                        ...
                        <div id="print">
                            <div id="left-middle">
                                <div id="product-list">
                                    ...
                                    <div style="clear:both;"></div>
                                <span class="text-danger" id="alert_location"></span>
                                </div>
                            </div>
                            <div style="clear:both;"></div>
                            <div id="left-bottom">
                                    ...
                                    <div class="row">
                                        <div class="col-xs-4" style="padding: 0;">
                                            <div class="btn-group-vertical btn-block">
                                                <button type="button" class="btn btn-warning btn-block btn-flat suspend-sale" id="suspend"><?=lang('suspend'); ?></button>
                                        ...
                                        <div class="col-xs-4" style="padding: 0;">
                                            <div class="btn-group-vertical btn-block">
                                                ...
                                                <!-- <a type="button" class="btn btn-info btn-block" id="print_order_receip" href="javascript:void(0)" onclick="or_print_re()">
                                                   
                                                </a>  -->

                                                <button type="button" class="btn btn-primary btn-block" id="print_bill" href="javascript:voice(0)" onclick="printBill()">
                                                    <?=lang('bill');?>
                                                </button>
                                                <!-- <button type="button" class="btn btn-primary btn-block" id="print_bill">
                                                  
                                                </button> -->
                                                <!-- <a type="button" class="btn btn-primary btn-block" id="p_bill" href="javascript:void(0)" onclick="p_bill()">
...
<div class="rotate btn-cat-con">
    <button type="button" id="open-brands" class="btn btn-info open-brands "><?= lang('table'); ?></button>
    ...
    <div id="brands-list">
        <?php
            foreach ($tables as $table) {
                ...
                echo '<button type="button" id="" value="'.$table->id.'" class="btn-prni ltable">';
                echo '<img src="'.site_url('assets/uploads/thumbs/no_image.png').'" style="width:60px;height:60px;" class="img-rounded img-thumbnail">';
                echo '<span>'.$table->name.'</span>';
                echo '</button>';
            }
        ?>
    </div>
                            ...
                            <div class="well well-sm well_1">
                                <div class="payment">
                                    <div class="row">
                                        ...
                                        <div class="col-sm-6">
                                            <div class="form-group">
                                                <label class="control-label">Amount (<?php if($Settings->default_currency=="USD"){echo "REL";}else{echo "USD";}?>)</label>
                                                <input type="hidden" name="ex_rate" value="<?php echo $currency->rate;  ?>" id="ex_rate">
                                                <input name="amountriel[]" type="text" id="amount_riel" class="pa form-control kb-pad amountriel"/>
                                                <label>( 1 US Dollar = <?php echo $this->sma->formatMoney($currency->rate); ?> Riels ) </label>
                                            </div>
                                        </div>
                        ...
                        <div class="font16">
                            <table class="table table-bordered table-condensed table-striped" style="margin-bottom: 0;">
                                ...
                                <tr>
                                    ...
                                    <td><span id="total_payable_r">0.00</span><span><?php if($Settings->default_currency=="USD"){echo "REL";}else{echo "USD";}?></span></td>
                                </tr>
                                <tr>
                                    ...
                                    <td><span id="balance_r">0.00</span><span><?php if($Settings->default_currency=="USD"){echo "REL";}else{echo "USD";}?></span></td>
                                </tr>
                                </tbody>
                            </table>
                            <div class="clearfix"></div>
...
<div class="modal fade in" id="dsModal" tabindex="-1" role="dialog" aria-labelledby="dsModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="form-group">
                    <!-- <?=lang("order_discount", "order_discount_input");?>(Without % = ) -->
                    <b>*Note</b>
                    <br>
                    <span style="font-size:11px;"> (With % = Disount in percentage)<br>(Without % = Discount in money)</span>
...
<!--  --><!-- <div id="bill_tbl"><span id="bill_span"></span>
    ...
    <table id="bill-total-table" class="prT table" style="margin-bottom:0;border:2px solid #000 !important;" width="100%"></table>
...
<div id="bill_tbl" >
  ...
  <center><table width="100%" class="prT" style="margin-bottom:0; border:1px solid #000000;">
    <thead>
...
<script type="text/javascript">
var site = <?=json_encode(array('url' => base_url(), 'base_url' => admin_url('/'), 'assets' => $assets,'munit'=>$un,'rate_cur'=> $currency->rate, 'settings' => $Settings, 'dateFormats' => $dateFormats))?>, pos_settings = <?=json_encode($pos_settings);?>;
var lang = {
...
<script type="text/javascript">
    var currency=<?php echo json_encode($currency->rate);?>;
</script>
...
<?php if ($this->input->get('customer')) {?>
        ...
        $(document).on('click', '.ltable',function(e){
            var ltable_id = $(this).val();

              // localStorage.setItem('poscustomer', ltable_id);

            // $('#poscustomer').val(ltable_id).trigger('change.select2');
            // alert(ltable_id);
             if (localStorage.getItem('positems')) {
                localStorage.removeItem('positems');
              }
        ...
        $(document).on('focus', '.amount, .amountriel', function () {
            pi = $(this).attr('id');
            calculateTotals();
        }).on('blur', '.amount, .amountriel', function () {
            calculateTotals();
        });
        <?php if($Settings->default_currency!="USD")
        {
            ?>
                function calculateTotals() {
                    ...
                    // });
                    if(ia.val()!='')
                        total_paying = formatDecimal(ia.val());
                    if($(".amountriel").val() != '') {
                        total_riel = $(".amountriel").val();
                    }

                    var rate=currency;
                    total_riel = formatDecimal(total_riel) * formatDecimal(rate);
                    total_riel = formatDecimal(total_riel);
                    total_paying = formatDecimal(total_paying) + formatDecimal(total_riel);
                    var toriel = formatDecimal(gtotal / rate);
                    $("#total_payable_r").text(formatMoney(toriel));

                    $("#amount_val_1").val(total_paying);

                    $('#total_paying').text(formatMoney(total_paying));
                    <?php if ($pos_settings->rounding) {?>
                        $('#balance').text(formatMoney(total_paying - round_total));
                        $('#balance_r').text(formatMoney((total_paying - round_total)  / rate));
                        $('#balance_' + pi).val(formatDecimal(total_paying - round_total));
                        $('#balance_amount_1').val(formatDecimal(total_paying - round_total));

                        total_paid = total_paying;
                        grand_total = round_total;
                    <?php } else {?>
                        $('#balance').text(formatMoney(total_paying - gtotal));
                        $('#balance_r').text(formatMoney(((total_paying -gtotal) / rate)+1));
                        $('#balance_' + pi).val(formatDecimal(total_paying - gtotal));
                        $('#balance_amount_1').val(formatDecimal(total_paying - gtotal));
                        total_paid = total_paying;
                        grand_total = gtotal;
                    <?php } ?>
                }
         ...
         <?php if($Settings->default_currency=="USD")
        {
            ...
            <?php if ($pos_settings->rounding) {?>
                $('#balance').text(formatMoney(total_paying - round_total));
                $('#balance_r').text(formatMoney((total_paying - round_total) * currency));
                $('#balance_' + pi).val(formatDecimal(total_paying - round_total));
                $('#balance_amount_1').val(formatDecimal(total_paying - round_total));
            ...
            <?php } else {?>
                $('#balance').text(formatMoney(total_paying - gtotal));
                $('#balance_r').text(formatMoney((total_paying - gtotal) * currency));
                $('#balance_' + pi).val(formatDecimal(total_paying - gtotal));
                $('#balance_amount_1').val(formatDecimal(total_paying - gtotal));
                total_paid = total_paying;
                grand_total = gtotal;
            <?php } ?>
                ...
                <?php
        ?>
         <?php }else if($Settings->default_currency!="USD")
        {
            ?>
                 function calculateTotals() {
            var total_paying = 0;
            ...
            <?php } else {?>
                $('#balance').text(formatMoney(total_paying - gtotal));
                $('#balance_r').text(formatMoney(((total_paying -gtotal) / rate)));
                $('#balance_' + pi).val(formatDecimal(total_paying - gtotal));
                $('#balance_amount_1').val(formatDecimal(total_paying - gtotal));
                total_paid = total_paying;
                grand_total = gtotal;
        ...
        $(document).on('click', '#submit-sale', function () {
                var us=$(".amount").val();
                var riel=$(".amountriel").val();
                // alert(riel);
                var note='Pay US='+us+', Pay riels='+riel;

            if (total_paid < grand_total) {
                bootbox.confirm("<?=lang('paid_l_t_payable');?>", function (res) {
                    if (res == true) {
                        $('#pos_note').val(localStorage.getItem('posnote')+note);
        ...
        //     }
        // });
        $('.suspend-sale').click(function () {
            ref = $('#reference_note').val("customert suspend");
            suspend = $('<span></span>');
            <?php if ($sid) { ?>
        ...
        $('#suspend_sale').click(function () {
            ref = $('#reference_note').val('customer suspend');
            // if (!ref || ref == '') {
...
<script type="text/javascript" src="<?=$assets?>pos/js/pos.ajax.js?<?php echo time();?>"></script>
<?php
...
<style type="text/css">
 ...
 <script type="text/javascript">
    ...
    function or_print_re(){
        con = confirm("Confirm Print");
        if (con===true) {
            do_capture_order_html();
        };
    }
     ...
     function do_capture_html(){
        var c = $('#bill_tbl');
            c.addClass('x2');
            div_content = document.querySelector("#bill_tbl")
            var options = {
                scale:2
            }
            //show data and capture and than hide
            if(!c.is(":visible")){
                c.show();
                c.fadeIn(1);
                c.fadeOut(1);
    ...
    function do_capture_order_html(){
        var ord= $('#order_tbl');
         ord.addClass('x2');
            div_content = document.querySelector("#order_tbl")
            var options = {
                scale:2
            }
            //show data and capture and than hide
