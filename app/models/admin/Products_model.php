...
class Products_model extends CI_Model
{
    ...
    {
        $this->db->select($this->db->dbprefix('products') . '.*, ' . $this->db->dbprefix('categories') . '.name as category,'.$this->db->dbprefix('location') . '.name as location')
        ->join('categories', 'categories.id=products.category_id', 'left')
        ->join('location','products.location=location.id');
    ...
    {
        $this->db->select($this->db->dbprefix('products') . '.*, ' . $this->db->dbprefix('tax_rates') . '.name as tax_rate_name, '.$this->db->dbprefix('tax_rates') . '.code as tax_rate_code, c.code as category_code, sc.code as subcategory_code,loc.name as location', FALSE)
            ...
            ->join('location loc', 'products.location=loc.id', 'left');
        $q = $this->db->get_where('products', array('products.id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
    ...
    public function addProduct($data, $items, $saleUnit, $warehouse_qty, $product_attributes, $photos)
    {
        if ($this->db->insert('products', $data)) {
            ...
            if ($items) {
             ...
             if ($saleUnit) {
                foreach ($saleUnit as $u_item) {
                   $u_item['product_id'] = $product_id;
                   $this->db->insert('saleunitprice',$u_item);
                }
            }
    ...
    public function updateProduct($id, $data,$saleUnit, $items, $warehouse_qty, $product_attributes, $photos, $update_variants)
    {
        if ($this->db->update('products', $data, array('id' => $id))) {
            ...
            if ($saleUnit) {
                $this->db->delete('saleunitprice', array('product_id' => $id));
                foreach ($saleUnit as $u_item) {
                   $u_item['product_id'] = $id;
                   $this->db->insert('saleunitprice',$u_item);
                }
            }
