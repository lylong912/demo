...
class Sales_model extends CI_Model
{
    ...
    public function getProductNames($term, $warehouse_id,$category_id, $limit = 25)
    {
        $wp = "( SELECT product_id, warehouse_id, quantity as quantity from {$this->db->dbprefix('warehouses_products')} ) FWP";

        $this->db->select('products.*, FWP.quantity as quantity, categories.id as category_id, categories.name as category_name,saleunitprice.sale_unitprice,saleunitprice.product_id as puid,saleunitprice.id as spid,location.name as location', FALSE)
            ->join($wp, 'FWP.product_id=products.id', 'left')
            
            ->join('categories', 'categories.id=products.category_id', 'left')
            ->join('saleunitprice','saleunitprice.product_id=products.id','left')
            ->join('location','location.id=products.location', 'left')
        ...
        if ($this->Settings->overselling) {
            ...
            if ($warehouse_id) {
                $this->db->where("(products.track_quantity = 1 AND FWP.quantity > 0) AND "
                . "({$this->db->dbprefix('products')}.name LIKE '%" . $term . "%' OR {$this->db->dbprefix('products')}.code LIKE '%" . $term . "%' OR  concat({$this->db->dbprefix('products')}.name, ' (', {$this->db->dbprefix('products')}.code, ')') LIKE '%" . $term . "%')");
            }else{
                $this->db->where("(products.track_quantity = 1 OR FWP.quantity > 0) AND "
                . "({$this->db->dbprefix('products')}.name LIKE '%" . $term . "%' OR {$this->db->dbprefix('products')}.code LIKE '%" . $term . "%' OR  concat({$this->db->dbprefix('products')}.name ' (', {$this->db->dbprefix('products')}.code, ')') LIKE '%" . $term . "%')");
            }
           
        }
        if ($category_id!='on' && $category_id != NULL) {
    ...
    {
        $this->db->select('sale_items.*, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, products.image, products.details as details, product_variants.name as variant,units.name as unit_name,units.name as uname')
    ...
    public function addSale($data = array(), $items = array(), $payment = array(), $si_return = array())
    {
        ...
        // =============================add for total item discount=============================
                $tid = $data['order_discount_id'];
                $uprice = $item['unit_price'];
                $i_qty = $item['unit_quantity'];
                $tp_qty = $i_qty * $uprice;
                // var_dump($tp_qty);die();
                    if (isset($tid)) {
                        $total_tid = $tid;
                        $tdpos = strpos($total_tid, $percentage);
                        if ($tdpos !== false) {
                            $tpds = explode("%", $total_tid);
                            $pr_tdis = $this->sma->formatDecimal(((($this->sma->formatDecimal($tp_qty)) * (Float) ($tpds[0])) / 100), 4);
                        }else{
                            $pr_tdis = $this->sma->formatDecimal($total_tid);
                        }
                    }
                    $item['invoice_dis'] = $data['order_discount_id'];
                    $item['item_invoice_dis'] = $pr_tdis;
        // =============================end add for total item discount=============================

                // var_dump($item['invoice_dis']);die();
    ...
    ///PAYMENT
    public function getRecordpayment(){
        return $this->db->query("SELECT pr.rec_id,c.company,pr.date,pr.amount_usd,pr.amount_r,pr.amount_b FROM sma_payment_record pr
                INNER JOIN sma_companies c ON pr.cus_id=c.id
                WHERE pr.delete=0 AND c.group_name='customer'")->result();
    }

    function getAllPaymentByCus($cus_id,$df,$dt){
        $where='';
        if ($df || $dt) {
        ...
        return $this->db->query("SELECT * FROM sma_sales WHERE customer_id = $cus_id AND payment_status<>'paid' {$where}")
                        ->result();
    }

    function get_selected_sale($cus_id,$sale_ids){
        $where =' AND(';
    ...
    public function save_payment_record($datarec){
        if($this->db->insert('sma_payment_record',$datarec)){
            return $this->db->insert_id();
        }
        return false;
    }
    public function save_record_detail($data){
        $this->db->insert('sma_payment_record_detail',$data);
    }

    public function get_sum_remain_cash($id){
        return $this->db->query("SELECT SUM(grand_total - paid) as balance FROM sma_sales WHERE customer_id = $id AND payment_status <> 'paid'")->row();
        }
