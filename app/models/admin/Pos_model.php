...
class Pos_model extends CI_Model
{
    ...
    //     $this->db->where('code','R');
    //     $this->db->select('*');
    //     $this->db->from('currencies');
    //     $q=$this->db->get();
    //     return $q->row();
    // }
    function getrielcurrency()
    {
        return $this->db->query("SELECT * FROM sma_currencies WHERE id = '3' ")->row();
    }
    ...
    {
        $this->db->select('products.*,location.name as location, warehouses_products.quantity, categories.id as category_id, categories.name as category_name')
    ...
    public function getAllInvoiceItems($sale_id)
    {
        if ($this->pos_settings->item_order == 0) {
            $this->db->select('sale_items.*,location.name as location, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, product_variants.name as variant, products.details as details')
    ...
    public function getSuspendedSaleItems($id)
    {
        $q = $this->db->query("SELECT sma_suspended_items.*,l.`name` as location
                                FROM sma_suspended_items
                                JOIN sma_products p
                                ON p.id = sma_suspended_items.product_id
                                JOIN sma_location l
                                ON l.id = p.location  where sma_suspended_items.suspend_id ='$id'");
    ...
    public function suspendSale($data = array(), $items = array(), $did = NULL, $customerid = NULL)
    {
        $cus = $this->db->query("SELECT customer_id, id FROM sma_suspended_bills WHERE customer_id = '$customerid' ")->row();
        // print_r($items);die();
        ...
        if(!empty($cus)) 
        {
            ...
            if($cus->customer_id == $customerid) 
            {
                ...
                $bill = array('suspend_id' => $cus->id);
                $total = array(
                    'total' => $data['grand_total'] + $this->db->where('customer_id', $customerid)->get('suspended_bills')->row()->total,
                    'count' => $data['total_items'] + $this->db->where('customer_id', $customerid)->get('suspended_bills')->row()->count
                );

                $this->db->where(array('id' => $cus->id, 'customer_id' => $data['customer_id']));
                if($this->db->update('suspended_bills', $total)) {
                    if($did) {
                        $this->db->where('suspend_id', $did)->delete('suspended_items');
                        foreach ($items as $row) {
                            // $proExist = $this->db->get_where('suspended_items', array('product_id'=>$row['product_id'], 'suspend_id'=>$did))->row();
                            // if(!empty($proExist)) {
                            //     $this->db->where('product_id', $proExist->product_id)->delete('suspended_items');
                            // }
                            $this->db->insert('suspended_items', array_merge($bill,$row));
                        }
                    } else {
                        foreach ($items as $row) {
                            $proExist = $this->db->get_where('suspended_items', array('product_id'=>$row['product_id'], 'suspend_id'=>$cus->id))->row();
                            if(!empty($proExist)) {
                                $this->db->where('product_id', $proExist->product_id);
                                $this->db->where('suspend_id', $cus->id);
                                $this->db->update('suspended_items', array('quantity' => $row['quantity'] + $proExist->quantity, 'unit_quantity' => $row['unit_quantity']+$proExist->unit_quantity, 'subtotal' => $proExist->subtotal + $row['subtotal']));
                                //$this->db->where('product_id', $proExist->product_id)->delete('suspended_items');
                            } else {
                                $this->db->insert('suspended_items', array_merge($bill,$row));
                            }
